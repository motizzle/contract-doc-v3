...
console.log('🔧 DEBUG: About to load analytics modules...');
const analyticsDb = require('./lib/analytics-db');
console.log('🔧 DEBUG: analytics-db loaded OK');
const geoip = require('geoip-lite');
console.log('🔧 DEBUG: geoip-lite loaded OK');

// Initialize analytics storage (MongoDB or JSON fallback)
const MONGODB_URI = process.env.MONGODB_URI || process.env.MONGO_URI;
console.log('🔧 DEBUG: Starting analytics initialization...');
console.log('🔧 DEBUG: MONGODB_URI exists:', !!MONGODB_URI);
console.log('🔧 DEBUG: dataAppDir:', dataAppDir);
analyticsDb.initialize(MONGODB_URI, dataAppDir)
  .then(() => {
    console.log('🔧 DEBUG: Analytics initialize() completed successfully');
...
app.get('/api/analytics/stats', async (req, res) => {
  try {
    // Support date range query parameter (default: 30 days)
    const days = parseInt(req.query.days) || 30;
    const stats = await analyticsDb.getStats({ days });
    res.json(stats);
  } catch (error) {
...
app.get('/analytics', (req, res) => {
  res.sendFile(path.join(webDir, 'analytics-pro.html'));
});

// Enhanced analytics dashboard (alternative view)
app.get('/analytics/enhanced', (req, res) => {
...
app.post('/api/v1/status/set', (req, res) => {
  try {
    const allowedStatuses = ['working draft', 'staff review', 'external review', 'final approval', 'pending signature', 'fully executed'];
    const newStatus = String(req.body?.status || 'working draft').toLowerCase();
