@echo off
echo ========================================
echo  WordFTW - Local Dev (AI DEMO MODE)
echo ========================================
echo.
echo 🎭 AI Demo Mode: Chat will show jokes instead of using Ollama
echo.
...
REM Set up environment - WITH AI DEMO MODE
echo [4/8] Setting up environment...
set LLM_PROVIDER=ollama
set OLLAMA_MODEL=gemma3:1b
set OLLAMA_BASE_URL=http://localhost:11434
set AI_DEMO_MODE=true
echo   - Environment configured (AI DEMO MODE)
echo.
...
if %ERRORLEVEL% EQU 0 (
  ...
  echo   - WARNING: Restart server to apply AI_DEMO_MODE
) else (
  echo   - Starting main server in AI DEMO MODE...
  start /MIN powershell -NoProfile -ExecutionPolicy Bypass -Command "$env:AI_DEMO_MODE='true'; cd '%SCRIPT_DIR%..\..\server'; npm start"
...
if %SIDELOAD_EXIT% EQU 0 (
  ...
  echo.
  echo To test AI demo:
  echo   1. Click the AI tab
  echo   2. Ask any question
  echo   3. You'll get a joke instead of AI response
  echo.
  echo To stop:
